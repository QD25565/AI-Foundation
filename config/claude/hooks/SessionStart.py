#!/usr/bin/env python3
"""SessionStart Hook - Thin wrapper for session-start binary.

Architecture principle:
  session-start (Rust CLI) = Ground truth. All logic lives here.
  This Python file = Adapter for systems that require Python (like Claude Code hooks).

No logic duplication. This file ONLY calls the binary and passes through output.
"""
import subprocess
import sys
import os
...
def main():
    ...
    try:
        ...
        print('<system-reminder>SessionStart: session-start timed out</system-reminder>')
