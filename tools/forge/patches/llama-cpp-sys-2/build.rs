...
fn extract_lib_names(out_dir: &Path, build_shared_libs: bool) -> Vec<String> {
    // Check if target is GNU (MinGW) - uses .a files not .lib
...
fn extract_lib_assets(out_dir: &Path) -> Vec<PathBuf> {
    let target = std::env::var("TARGET").unwrap_or_default();
    let is_gnu = target.contains("gnu");
    let shared_lib_pattern = if cfg!(windows) && !is_gnu {
        "*.dll"
    } else if cfg!(target_os = "macos") {
        "*.dylib"
    } else {
    ...
    let shared_libs_dir = if cfg!(windows) && !is_gnu { "bin" } else { "lib" };
...
fn main() {
    ...
    // Fix for MinGW: create lib-prefixed copies of .a files that lack the prefix
                            ...
                            if let Some(name) = path.file_name() {
                                ...
                                if !name_str.starts_with("lib") {
                                    ...
                                    if !new_path.exists() {
                                        ...
                                        debug_log!("Created lib-prefixed copy: {}", new_name);
                                    }
                                }
                            }
                        }
    ...
    match target_os {
        ...
        TargetOs::Windows(WindowsVariant::Other) => {
            ...
            if let Ok(mingw_path) = std::env::var("MINGW_LIB_PATH") {
                println!("cargo:rustc-link-search=native={}", mingw_path);
            } else {
                // MINGW_LIB_PATH must be set for GNU target builds
                // Install LLVM-MinGW and set: MINGW_LIB_PATH=/path/to/llvm-mingw/x86_64-w64-mingw32/lib
                eprintln!("cargo:warning=MINGW_LIB_PATH not set! Build may fail for GNU target.");
                eprintln!("cargo:warning=Install LLVM-MinGW and set MINGW_LIB_PATH environment variable.");
            }
