...
//! - `sage-724@my-desktop` — by Teambook name
...
pub struct FederatedAiEntry {
    ...
    /// Human-readable Teambook name (e.g., "my-desktop")
...
impl AiRegistry {
    ...
    /// (`sage-724@my-desktop` or `sage-724@a3f7c2d1`).
...
mod tests {
    ...
    async fn test_resolve_with_local_qualifier() {
        ...
        assert!(matches!(
            registry.resolve("sage-724@my-desktop").await,
            AiResolution::Local
        ));
    }
