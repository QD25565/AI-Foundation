//! PyO3 bindings for Python integration (PyO3 0.22 compatible)
...
impl PyDigitalPheromone {
    #[new]
    fn new(location: String, pheromone_type: String, intensity: f64, decay_rate: f64, agent_id: String) -> PyResult<Self> {
...
impl PyStigmergyServer {
    ...
    #[pyo3(signature = (location, pheromone_type=None))]
    fn sense_environment(&self, py: Python, location: String, pheromone_type: Option<String>) -> PyResult<PyObject> {
        ...
        // Manually construct Python list to avoid ToPyObject trait issue
        let py_list = PyList::empty_bound(py);
        for pheromone in pheromones {
            let py_pheromone = Py::new(py, PyDigitalPheromone { inner: pheromone })?;
            py_list.append(py_pheromone)?;
        }
    ...
    #[pyo3(signature = (location, pheromone_type=None))]
    fn get_intensity(&self, location: String, pheromone_type: Option<String>) -> PyResult<f64> {
        let ptype = if let Some(t) = pheromone_type {
            Some(str_to_pheromone_type(&t)?)
        } else {
            None
    ...
    fn refresh_pheromone(&self, location: String, pheromone_type: String, agent_id: String) -> PyResult<bool> {
        let ptype = str_to_pheromone_type(&pheromone_type)?;
    ...
    #[pyo3(signature = (location, pheromone_type, agent_id=None))]
    fn clear_pheromone(&self, location: String, pheromone_type: String, agent_id: Option<String>) -> PyResult<()> {
        let ptype = str_to_pheromone_type(&pheromone_type)?;
...
#[pymodule]
fn stigmergy_core(m: &Bound<'_, PyModule>) -> PyResult<()> {
