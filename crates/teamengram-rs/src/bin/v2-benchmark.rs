...
fn benchmark_event_log_append(data_dir: &PathBuf) {
    ...
    for _ in 0..WARMUP_ITERATIONS {
        let event = Event::broadcast("lyra-584", "warmup", "test");
    ...
    for i in 0..BENCHMARK_ITERATIONS {
        let event = Event::broadcast("lyra-584", "general", &format!("msg{}", i));
...
fn benchmark_view_sync(data_dir: &PathBuf) {
    ...
    // Create view engine
    let mut view = ViewEngine::open("lyra-584", &view_dir).unwrap();
    let mut reader = EventLogReader::open(Some(&log_dir)).unwrap();
