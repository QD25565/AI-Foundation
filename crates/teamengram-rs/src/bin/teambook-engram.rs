...
enum Commands {
    ...
    // Locks removed — deprecated (Feb 2026, architecture decision). Use file claims instead.
    ...
    // Stigmergy (StigmergySense, StigmergyDeposit) removed — deprecated (Feb 2026, architecture decision)
...
async fn main() -> Result<()> {
    ...
    if ai_id.is_empty() || ai_id == "unknown" {
        ...
        eprintln!("Set a valid AI identity, e.g.: AI_ID=ai-3");
        std::process::exit(1);
    }
    ...
    match cli.command {
        ...
        Commands::Status => {
            ...
            for p in presences {
                // Use clear words instead of ambiguous symbols (architecture decision)
...
fn run_v2(ai_id: &str, command: Commands) -> Result<()> {
    ...
    match command {
        ...
        Commands::LogAction { action, path } => {
            ...
            // Stigmergy pheromone auto-deposit removed (Feb 2026, architecture decision)
            ...
            if let Ok(file_actions) = v2.get_file_actions(file_limit) {
                let file_strs: Vec<String> = file_actions.iter()
                    ...
                    .map(|(ai, action, path, _)| {
                        format!("{} {} {}", ai, action, path)  // Full: "ai-2 modified /path/to/file.rs"
                    })
                    .collect();
