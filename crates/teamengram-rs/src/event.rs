...
pub mod event_type {
    ...
    // Lock Events (0x04XX) — DEPRECATED: Locks removed (Feb 2026, architecture decision)
    ...
    // Stigmergy Events (0x07XX) — DEPRECATED: Stigmergy removed (Feb 2026, architecture decision)
...
impl Event {
    ...
    // lock_acquire() and lock_release() removed — locks deprecated (Feb 2026, architecture decision)
    ...
    // pheromone_deposit() removed — stigmergy deprecated (Feb 2026, architecture decision)
...
mod tests {
    ...
    fn test_event_roundtrip() {
        let event = Event::broadcast("ai-1", "general", "Hello, team!");
        let bytes = event.to_bytes();
        let decoded = Event::from_bytes(&bytes).expect("Should decode");

        assert_eq!(decoded.source_ai(), "ai-1");
    ...
    fn test_event_checksum_validation() {
        let event = Event::direct_message("ai-2", "ai-1", "Test message");
