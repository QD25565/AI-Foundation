...
//!
//! Ported from ai-foundation-clean/src/hlc.rs.
...
///
/// Thread-safe via internal Mutex. Guarantees:
...
impl HybridClock {
    ...
    ///
    /// `node_id` should come from `TeambookIdentity::hlc_node_id()` — first 8 bytes
    /// of the Ed25519 public key as u64. Globally unique without coordination.
    ...
    /// Derive an HLC node_id from an Ed25519 public key (first 8 bytes as u64).
    ...
    ///
    /// Returns `Ok(new_timestamp)` guaranteed > both local state and remote timestamp.
    /// Returns `Err` if the remote timestamp's physical time exceeds local time
    /// by more than MAX_DRIFT_US (60 seconds).
...
mod tests {
    ...
    fn test_receive_rejects_excessive_drift() {
        ...
        assert!(clock.receive(&far_future).is_err());
    }
    ...
    #[test]
    fn test_ordering() {
        let a = HlcTimestamp { physical_time_us: 100, counter: 5, node_id: 1 };
        let b = HlcTimestamp { physical_time_us: 100, counter: 5, node_id: 2 };
        let c = HlcTimestamp { physical_time_us: 100, counter: 6, node_id: 1 };
        let d = HlcTimestamp { physical_time_us: 101, counter: 0, node_id: 1 };
        assert!(a < b); // same time/counter, higher node_id wins
        assert!(a < c); // same time, higher counter wins
        assert!(c < d); // higher physical time wins
    }
